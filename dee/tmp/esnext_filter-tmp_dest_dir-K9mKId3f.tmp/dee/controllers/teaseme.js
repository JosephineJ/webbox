...
var TeaseMeController = Ember.ArrayController.extend({
	...
	init: function() {
		this._super();
		var cthis = this;
		this.growingCode = ""; /*Insert first character of used model 
		string here, as chooseLetter will return undefined in first 
		execution round instead - Outdated ...fixed with ember run function in init */
        //this.chooseLetter();
	...
	chooseLetterMotto: function() {
		...
		if (j <= len){
			...
			tthis.set('buttonText',"Find out more");
			tthis.set('showButton',true);
			return false;
		}
	},
});
